...
class CacheResponse {
  ...
  static Future<CacheResponse> fromResponse(String key, Response response,
      DateTime expiry, CachePriority priority) async {
    final content =
        await _serializeData(response.request.responseType, response.data);
  ...
  void updateRequest(RequestOptions options, bool addModifiedHeaders) {
    if (addModifiedHeaders) {
      options.headers['If-Modified-Since'] = downloadedAt;
      if (eTag != null) {
  ...
  static Future<List<int>> _serializeData(
      ResponseType type, dynamic data) async {
    ...
    throw UnsupportedError(
        'Not supported ResponseType : $type. Please file an issue on repository to add compatibility');
  }
